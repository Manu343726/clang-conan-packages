...
COMPONENT=$1
VERSION=$2
CUSTOM_COMPONENT_PACKAGE_NAME=$3

if [[ -z "$CUSTOM_COMPONENT_PACKAGE_NAME" ]]; then
    CUSTOM_COMPONENT_PACKAGE_NAME=$COMPONENT
fi

URL=http://releases.llvm.org/$VERSION/$CUSTOM_COMPONENT_PACKAGE_NAME-$VERSION.src.tar.xz
TMP_DIR=`mktemp -d`
